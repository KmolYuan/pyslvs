...
inline double point_on_line(Constraint &con) {
    ...
    double t = ((P1_x - L1_P1_x) * dx + (P1_y - L1_P1_y) * dy) / (dx * dx + dy * dy);
...
inline double colinear(Constraint &con) {
    ...
    if (m <= 1 && m > -1) {
        ...
        return (e1 - L2_P1_y) * (e1 - L2_P1_y) + (e2 - L2_P2_y) * (e2 - L2_P2_y);
    } else {
        ...
        return (e1 - L2_P1_x) * (e1 - L2_P1_x) + (e2 - L2_P2_x) * (e2 - L2_P2_x);
    }
}
